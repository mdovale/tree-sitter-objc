; Objective-C highlights: upstream uses `; inherits: c` — bundled SPM queries are a single file, so Swift/Neon never expands inheritance. Base C rules are inlined from tree-sitter-c (Slates vendored copy) so keywords, strings, types, etc. highlight like C.
...
; --- Objective-C (tree-sitter-objc extensions) ---
